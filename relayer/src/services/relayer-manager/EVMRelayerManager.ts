  ...
  async postTransactionMined(relayerAddress: string): Promise<void> {
    ...
    if (relayerData) {
      ...
      if (balance.lt(this.fundingBalanceThreshold)) {
        log.info(`Balance of relayer ${address} is less than threshold ${this.fundingBalanceThreshold} on chainId: ${this.chainId} from ULR ${this.networkService.getActiveRpcUrl()}`);
        try {
