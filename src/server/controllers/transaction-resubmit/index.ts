...
export const transactionResubmitApi = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (relayer) {
      ...
      log.info(
        `Resubmitting transaction ${transactionId} with gasPrice ${gasPrice} on chainId ${chainId} and raw transaction data ${JSON.stringify(
          rawTransaction,
        )}`,
      );
      ...
      log.info(
        `Resubmitted transaction ${transactionId} on chainId ${chainId} and result ${JSON.stringify(
          result,
        )}`,
      );
      if (result.success) {
