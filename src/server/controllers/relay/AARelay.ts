...
export const relayAATransaction = async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      log.info(
        `dappAPIKey: ${dappAPIKey} for userOp: ${JSON.stringify(userOp)}`,
      );
      const {
      ...
      metaData.destinationSmartContractAddresses =
        destinationSmartContractAddresses;
      metaData.destinationSmartContractMethods =
        destinationSmartContractMethods;
      log.info(
        `MetaData to be saved: ${JSON.stringify(
          metaData,
      ...
      log.error(
        `Error in getting meta data from userOp: ${JSON.stringify(userOp)}`,
      );
