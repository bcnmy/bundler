...
export const estimateUserOperationGas = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const simulator = getBundlerSimulationServiceMap(parseInt(chainId, 10));
    if (!simulator) {
      return res
        .status(STATUSES.BAD_REQUEST)
    ...
    const gasPriceService = getGasPriceServiceMap(parseInt(chainId, 10));
    if (!gasPriceService) {
      return res
        .status(STATUSES.BAD_REQUEST)
