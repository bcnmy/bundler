...
describe("Network Service: Rpc Urls", () => {
  it("Main rpc url should be active for chainId: 5", async () => {
    const { data } = await networkServiceMap[5].sendRpcCall(
      "eth_blockNumber",
      []
    );
  ...
  it("Main rpc url should be active for chainId: 80001", async () => {
    const { data } = await networkServiceMap[80001].sendRpcCall(
      "eth_blockNumber",
      []
    );
    const blockNumber = Number(BigNumber.from(data.result));
  ...
  it("Fallback urls should be active for chainId: 5", async () => {
    for (
      ...
      const fallBackRpcUrl =
        networkServiceMap[5].fallbackRpcUrls[fallBackRpcUrlIndex];
  ...
  it("Fallback urls should be active for chainId: 80001", async () => {
    for (
      ...
      const fallBackRpcUrl =
        networkServiceMap[80001].fallbackRpcUrls[fallBackRpcUrlIndex];
      log.info(`Checking rpcUrl: ${fallBackRpcUrl}`);
...
describe("Network Service: Native Asset Balance", () => {
  it("Fetches the correct native asset balance on chainId: 80001", async () => {
    ...
    const ownerAddressPrivateKey =
      "e3b3818b1b604cf6dfc3133faa9a524f1e2ea0d5894a003c4b857952f6b146f6";
    ...
    const walletBalance = Number(
      await networkServiceMap[80001].getBalance(wallet.address)
    );
  ...
  it("Fetches the correct native asset balance on chainId: 5", async () => {
    ...
    const ownerAddressPrivateKey =
      "d952fa86f1e2fed30fb3a6da6e5c24d7deb65b6bb46da3ece5f56fd39e64bbd0";
    const ownerWallet = new ethers.Wallet(
      ownerAddressPrivateKey,
    ...
    // check if the getBalance gets 0.00001
    const walletBalance = Number(
      await networkServiceMap[5].getBalance(wallet.address)
    );
...
describe("Network Service: Nonce Check", () => {
  it("Check if nonce is correctly incremented on chainId: 80001", async () => {
    ...
    const nonceBeforeTransaction = await networkServiceMap[80001].getNonce(
    ...
    const nonceAfterTransaction = await networkServiceMap[80001].getNonce(
      ownerAddressPublicKey
    );
  ...
  it("Check if nonce is correctly incremented on chainId: 5", async () => {
    ...
    const nonceBeforeTransaction = await networkServiceMap[5].getNonce(
      ownerAddressPublicKey
    );
    const { gasPrice } = await networkServiceMap[5].getGasPrice();
    ...
    // then call getNonce() on that address, nonce should x + 1
    const nonceAfterTransaction = await networkServiceMap[5].getNonce(
      ownerAddressPublicKey
    );
    const nonceDifference = nonceAfterTransaction - nonceBeforeTransaction;
    expect(nonceDifference).toBe(1);
...
describe("Network Service: Sending Transaction", () => {
  it("Transaction should be sent and confirm on chainId: 80001", async () => {
    ...
    const { gasPrice } = await networkServiceMap[80001].getGasPrice();
    const nonce = await networkServiceMap[80001].getNonce(
      ownerAddressPublicKey
    );
    ...
    const transactionExecutionResponse =
      await networkServiceMap[80001].sendTransaction(
        rawTransactionData,
        evmAccount
      );
    ...
    const transactionReceipt =
      await networkServiceMap[80001].waitForTransaction(hash);
  ...
  it("Transaction should be sent and confirm on chainId: 5", async () => {
    ...
    const transactionExecutionResponse =
      await networkServiceMap[5].sendTransaction(
        rawTransactionData,
        evmAccount
      );
    ...
    const transactionReceipt = await networkServiceMap[5].waitForTransaction(
      hash
    );
    expect(transactionReceipt).not.toBeNull();
    expect(transactionReceipt.status).toBe(1);
