  ...
  async executeTransaction(
    ...
    const retryExecuteTransaction = async (
      ...
      try {
        ...
        log.info(
          `Sending transaction to network: ${JSON.stringify(
            rawTransaction
          )} for bundler address: ${
            rawTransaction.from
          } for transactionId: ${transactionId} on chainId: ${this.chainId}`
        ...
        log.info(
          `Transaction execution response: ${JSON.stringify(
        ...
        if (NONCE_TOO_LOW.some((str) => errInString.indexOf(str) > -1)) {
          ...
          const bumpedUpGasPrice = await this.handleReplacementFeeTooLow(
            rawTransaction
          );
    ...
    try {
      ...
      if (transactionType === TransactionType.BUNDLER) {
        ...
        this.userOperationStateDao.updateState(this.chainId, {
          transactionId,
          transactionHash:
            transactionExecutionResponse.transactionResponse.hash,
    ...
    try {
      ...
      if (typeof bumpedUpGasPrice === "string") {
        ...
        rawTransaction.maxPriorityFeePerGas =
          bumpedUpGasPrice.maxPriorityFeePerGas;
        log.info(
          `Bumped up gas price for transactionId: ${transactionId} is ${JSON.stringify(
            bumpedUpGasPrice
          )} on chainId ${this.chainId}`
        );
      }
      ...
      log.info(
        `Raw transaction for retrying transactionId: ${transactionId} is ${JSON.stringify(
          rawTransaction
        )} on chainId ${this.chainId}`
      );
      ...
      if (retryTransactionExecutionResponse.success) {
        ...
        await this.sendTransactionFailedSlackNotification(
          transactionId,
          this.chainId,
          retryTransactionExecutionResponse.error
        );
        return {
          state: "failed",
          code: STATUSES.INTERNAL_SERVER_ERROR,
