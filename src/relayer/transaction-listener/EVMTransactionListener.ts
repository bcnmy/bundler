    ...
    ITransactionPublisher<TransactionQueueMessageType>
{
    ...
    if (transactionExecutionResponse) {
        ...
        if (
          ...
          for (
            ...
            if (entryPointContract) {
              ...
              log.info(
                `Updating userOp data: ${JSON.stringify({
                  transactionHash: transactionExecutionResponse?.hash,
                  receipt: transactionReceipt,
      ...
      try {
        ...
        if (
          ...
          log.info(
            `gasUsed or effectiveGasPrice field not found in ${JSON.stringify(
              transactionExecutionResponse
            )}`
          );
        } else {
          ...
          for (
            ...
            if (entryPointContract) {
              ...
              if (
                ...
                const coinsRateObj = await this.cacheService.get(
                  getTokenPriceKey()
                );
                if (!coinsRateObj) {
  ...
  async notify(
    ...
    if (
      !transactionExecutionResponse ||
      Object.keys(transactionExecutionResponse).length === 0
    ) {
    ...
    try {
      ...
      log.error(
        ...
        )} Timeout hit for waiting on hash: ${
          transactionExecutionResponse.hash
        } for transactionId: ${transactionId} on chainId ${this.chainId}`
      );
      return {
  ...
  async getTransactionFailureMessage(
    ...
    try {
      ...
      const { from, to, data, gasLimit, gasPrice } = getTransactionResponse;

      const gasInHex = BigNumber.from(gasLimit)
        .toHexString()
        .substring(2)
        .replace(/^0+/, "");
      const gasPriceInHex = BigNumber.from(gasPrice)
        .toHexString()
        .substring(2)
        .replace(/^0+/, "");

      const handleOpResult = await this.networkService.sendRpcCall("eth_call", [
        {
          from,
          to,
          data,
