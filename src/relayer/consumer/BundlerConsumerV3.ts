...
import { BundlerV3ConsumerParamsType } from "./types";
import { STATUSES } from "../../server/api/shared/middleware";
import { ENTRY_POINT_V07_ABI } from "../../common/entrypoint-v7/abiv7";
import { packUserOperation } from "../../common/entrypoint-v7/PackedUserOperation";

const log = logger.child({
  module: module.filename.split("/").slice(-4).join("/"),
});
export class BundlerConsumerV3
  implements ITransactionConsumer<IEVMAccount, EVMRawTransactionType>
{
  private transactionType: TransactionType = TransactionType.BUNDLER_V3;

  private queue: IQueue<BundlerV3TransactionMessageType>;
  ...
  constructor(bundlerConsumerParamas: BundlerV3ConsumerParamsType) {
      ...
      const sendTransactionWithRetry = async (): Promise<void> => {
        ...
        if (activeRelayer) {
          ...
          const data = encodeFunctionData({
            ...
            args: [[packedUserOp], activeRelayer.getPublicKey() as `0x${string}`],
          });
          log.info("after encodeFunctionData");
          ...
          log.info("after await this.cacheService.set");
          ...
          log.info("after await this.getFailedTransactionRetryCountKey");
          try {
