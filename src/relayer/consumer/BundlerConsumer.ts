    ...
    if (msg) {
      ...
      const sendTransactionWithRetry = async (): Promise<void> => {
        ...
        if (activeRelayer) {
          ...
          log.info(
            `Setting active relayer: ${activeRelayer?.getPublicKey()} as beneficiary for userOp: ${JSON.stringify(
              userOp
            )} for transactionId: ${
              transactionDataReceivedFromQueue.transactionId
            } on chainId: ${this.chainId}`
          ...
          const { data } = await (
            entryPointContract as ethers.Contract
          ).populateTransaction.handleOps(
            [userOp],
          ...
          try {
            ...
            log.error(
              `Error in transaction service for transactionId: ${
                ...
                this.chainId
              } with error: ${JSON.stringify(parseError(error))}`
            );
            log.info(
              `Adding relayer: ${activeRelayer.getPublicKey()} back to active relayer queue for transactionId: ${
                transactionDataReceivedFromQueue.transactionId
