...
export class BundlerSimulationService {
  ...
  async estimateUserOperationGas(
    ...
    try {
      ...
      log.info(
        `userOp received: ${JSON.stringify(userOp)} on chainId: ${chainId}`,
      );
      ...
      log.info(
        `userOp to used to simulate in eth_call: ${JSON.stringify(
          userOp,
        )} on chainId: ${chainId}`,
      );

      const { data } =
        await entryPointContract.populateTransaction.simulateHandleOp(
          userOp,
          config.zeroAddress,
          "0x",
        );
      ...
      try {
        ...
        log.info(
          `eth_call response: ${JSON.stringify(
            simulateHandleOpResult.data,
          )} on chainId: ${chainId}`,
        );
        ...
        if (ethCallData === undefined) {
          const errorMessage = simulateHandleOpResult.data.error.message
            ? simulateHandleOpResult.data.error.message
            : "eth_call RPC error";
          return {
      ...
      const errorDescription =
        entryPointContract.interface.parseError(ethCallData);
      ...
      if (errorDescription.name === "ExecutionResult") {
        ...
        const verificationGasLimit =
          Math.ceil((preOpGas - preVerificationGas) * 1.2) + 5000;
        log.info(
        ...
        // }
        log.info(
          `call gas limit after checking for optimism: ${callGasLimit} on chainId: ${chainId}`,
        );

        if (LineaNetworks.includes(chainId)) {
          preVerificationGas += Math.ceil(
            (verificationGasLimit + callGasLimit) / 3,
          );
        }
  ...
  async simulateValidation(simulateValidationData: SimulateValidationData) {
    ...
    try {
      ...
      log.info(
        `ethEstimateGasParams: ${JSON.stringify(
      ...
      log.info(
        `Response from eth_estimateGas: ${JSON.stringify(
          ethEstimatGasResponse.data,
        )}`,
      );
      ...
      if (
        ...
        ethEstimatGasResponse.data.result &&
        Object.keys(ethEstimatGasResponse.data).length > 0
      ) {
  ...
  static parseSimulateHandleOpResult(
    ...
    if (!simulateHandleOpResult?.errorName?.startsWith("ExecutionResult")) {
      ...
      if (!simulateHandleOpResult.errorArgs) {
        throw new RpcError(
          `Error: ${JSON.stringify(simulateHandleOpResult)}`,
          BUNDLER_VALIDATION_STATUSES.WALLET_TRANSACTION_REVERTED,
        );
      }
