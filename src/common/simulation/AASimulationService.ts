...
export class AASimulationService {
  ...
  async simulate(
    ...
    log.info(
      `Entry Point address to be used for simulateValidation: ${entryPointContract.address} for chainId: ${chainId}`,
    );
    let isSimulationSuccessful = true;
    if (
      entryPointContract.address.toLowerCase() ===
      "0x119df1582e0dd7334595b8280180f336c959f3bb"
    ) {
  ...
  static parseUserOpSimulationResult(
    ...
    if (!simulationResult?.errorName?.startsWith("ValidationResult")) {
      log.info(
        `Inside ${!simulationResult?.errorName?.startsWith(
          "ValidationResult",
        )}`,
      );
