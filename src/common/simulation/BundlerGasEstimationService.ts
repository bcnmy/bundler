...
export class BundlerGasEstimationService {
  ...
  async estimateCreationGas(
    ...
    return this.networkService
      ...
      .catch((error) => {
        const message =
          error.message.match(/reason="(.*?)"/)?.at(1) ?? "execution reverted";
  ...
  async estimateUserOperationGas(
    ...
    if (userOp.callData === "0x") {
      ...
      callGasLimit = await this.networkService
        ...
        .catch((error) => {
          const message =
            error.message.match(/reason="(.*?)"/)?.at(1) ??
            "execution reverted";
          log.info(`message: ${JSON.stringify(message)}`);
          return 0;
        });
    ...
    const validateUserOpGas =
      DefaultGasLimits.validatePaymasterUserOpGas +
    ...
    const preVerificationGas =
      await BundlerGasEstimationService.calcPreVerificationGas(
        simulateUserOp,
        chainId,
        entryPointContract,
      );
  ...
  static async calcPreVerificationGas(
    ...
    if (
      ...
      const data = await calcOptimismPreVerificationGas(userOp, chainId);
      ret += data;
    }
    ret += 30000;
    return ret;
