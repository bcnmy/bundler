...
import {
  GetMetaDataFromUserOpReturnType,
  Log,
  StakeInfo,
  UserOperationType,
} from "../types";
...
export const getMetaDataFromUserOp = async (
  ...
  try {
    ...
    log.info(
      `Extracting data for wallet address: ${walletAddress} for dappAPIKey: ${dappAPIKey} for userOp: ${JSON.stringify(
        userOp,
      )}`,
    );
    const destinationSmartContractAddresses: Array<string> = [];
    const destinationSmartContractMethodsCallData: Array<string> = [];
    const destinationSmartContractMethods: Array<{
      address: string;
      name: string;
    }> = [];
    ...
    log.info(
      `Multi Send Contract Address: ${multiSendContractAddress} for dappAPIKey: ${dappAPIKey} for userOp: ${JSON.stringify(
        userOp,
      )}`,
    );

    const { callData, sender } = userOp;
    const iFaceSmartWallet = new ethers.utils.Interface(
      JSON.stringify(smartWalletAbi),
    );
    const decodedDataSmartWallet = iFaceSmartWallet.parseTransaction({
      data: callData,
    });
    log.info(
      `Decoded smart wallet data: ${JSON.stringify(
        decodedDataSmartWallet,
      )} for dappAPIKey: ${dappAPIKey} for userOp: ${JSON.stringify(userOp)}`,
    );

    const smartWalletExecFunctionName = decodedDataSmartWallet.name;
    log.info(
      `Name of smart wallet method: ${JSON.stringify(
        smartWalletExecFunctionName,
      )} for dappAPIKey: ${dappAPIKey} for userOp: ${JSON.stringify(userOp)}`,
    );
    ...
    if (smartWalletExecFunctionName === "executeCall") {
      ...
      if (!methodArgsSmartWalletExecuteCall) {
        log.info(
          `No value args found in decoded data of the smart wallet for executeCall for dappAPIKey: ${dappAPIKey} for userOp: ${JSON.stringify(
            userOp,
          )}`,
        );
        return {
      ...
      log.info(
        `Arguments of smart wallet method: ${JSON.stringify(
          methodArgsSmartWalletExecuteCall,
        )} for dappAPIKey: ${dappAPIKey} for userOp: ${JSON.stringify(userOp)}`,
      );
      if (
        !(
          methodArgsSmartWalletExecuteCall[0].toLowerCase() ===
          sender.toLowerCase()
        )
      ) {
        log.info(
          `Destination address is not sender's wallet for dappAPIKey: ${dappAPIKey} for userOp: ${JSON.stringify(
            userOp,
          )}`,
        );
        ...
        log.info(
          `targetContractCode: ${targetContractCode} for dappAPIKey: ${dappAPIKey} for userOp: ${JSON.stringify(
            userOp,
          )}`,
        );
        if (!(targetContractCode === "0x")) {
          log.info(
            `Destination address is not EOA for dappAPIKey: ${dappAPIKey} for userOp: ${JSON.stringify(
              userOp,
            )}`,
          );
      ...
      if (!methodArgsSmartWalletExecuteBatchCall) {
        log.info(
          `No value args found in decoded data of the smart wallet for executeBatchCall for dappAPIKey: ${dappAPIKey} for userOp: ${JSON.stringify(
            userOp,
          )}`,
        );
        return {
      ...
      log.info(
        `Arguments of smart wallet method: ${JSON.stringify(
          methodArgsSmartWalletExecuteBatchCall,
        )} for dappAPIKey: ${dappAPIKey} for userOp: ${JSON.stringify(userOp)}`,
      );
      for (
        let index = 0;
        ...
        if (targetContractCode === "0x") {
          log.info(
            `Destination address is EOA for dappAPIKey: ${dappAPIKey} for userOp: ${JSON.stringify(
              userOp,
            )}`,
          );
          continue;
        ...
        if (
          ...
          log.info(
            `Destination address is sender's wallet for dappAPIKey: ${dappAPIKey} for userOp: ${JSON.stringify(
              userOp,
            )}`,
          );
          continue;
      ...
      log.info(
        `User op has call data of: ${smartWalletExecFunctionName} which is not supported for dappAPIKey: ${dappAPIKey} for userOp: ${JSON.stringify(
          userOp,
        )}`,
      );
      return {
    ...
    log.info(
      `Destination Smart Contract Addresses for walletAddress: ${
        userOp.sender
      } are: ${destinationSmartContractAddresses} for dappAPIKey: ${dappAPIKey} for userOp: ${JSON.stringify(
        userOp,
      )}`,
    );
    log.info(
      `Destination Smart Contract Methods call data for walletAddress: ${
        userOp.sender
      } are: ${destinationSmartContractMethodsCallData} for dappAPIKey: ${dappAPIKey} for userOp: ${JSON.stringify(
        userOp,
      )}`,
    );
    log.info(
      `Getting smart contract data for addresses: ${JSON.stringify(
        destinationSmartContractAddresses,
      )} for dappAPIKey: ${dappAPIKey} for userOp: ${JSON.stringify(userOp)}`,
    );
    log.info(
      `Making call to paymaster dashboard backend to get data on ${
    ...
    log.info(
      `Respone from paymaster dashboard backend: ${JSON.stringify(
        dataFromPaymasterDashboardBackend,
      )} for dappAPIKey: ${dappAPIKey} for userOp: ${JSON.stringify(userOp)}`,
    );
    if (dataFromPaymasterDashboardBackend.statusCode !== STATUSES.SUCCESS) {
      throw dataFromPaymasterDashboardBackend.message;
    }
    const { dapp, smartContracts } = dataFromPaymasterDashboardBackend.data;
    // const dappId = dapp._id;
    log.info(dataFromPaymasterDashboardBackend.data);

    log.info(
      `Data fetched for dappAPIKey: ${dappAPIKey} for userOp: ${JSON.stringify(
        userOp,
      )}`,
    );
    log.info(
    ...
    log.info(
      `Smart Contracts: ${JSON.stringify(
        smartContracts,
      )} for dappAPIKey: ${dappAPIKey} for userOp: ${JSON.stringify(userOp)}`,
    );
    ...
    for (
      ...
      for (
        ...
        if (
          destinationSmartContractAddresses[smartContractDataIndex] ===
    ...
    log.info(
      `Extracting data for: ${destinationSmartContractAddresses.length} contract method calls`,
    );
    for (
      let smartContractDataIndex = 0;
      smartContractDataIndex < destinationSmartContractAddresses.length;
      ...
      const destinationSmartContractMethodCallData =
        destinationSmartContractMethodsCallData[smartContractDataIndex];
    ...
    log.info(
      `Destination Smart Contract Addresses: ${JSON.stringify(
        destinationSmartContractAddresses,
      )} for dappAPIKey: ${dappAPIKey} for userOp: ${JSON.stringify(userOp)}`,
    );
    log.info(
      `Destination Smart Contract Methods: ${JSON.stringify(
        destinationSmartContractMethods,
      )} for dappAPIKey: ${dappAPIKey} for userOp: ${JSON.stringify(userOp)}`,
    );
    ...
    log.error(
      `Error in getting wallet transaction data for userOp: ${userOp} on chainId: ${chainId} with error: ${parseError(
        error,
  ...
  try {
    ...
    try {
      ...
      if (event[0]) {
        ...
        log.info(
          `userOperationEventArgs: ${JSON.stringify(userOperationEventArgs)}`,
        );
  ...
  try {
    ...
    for (const eventLog of logs) {
      ...
      if (
        ...
        eventLog.topics[1].toLowerCase() === userOpHash.toLowerCase()
      ) {
