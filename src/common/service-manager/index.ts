  ...
  for (const chainId of supportedNetworks) {
    ...
    for (const relayerManager of config.relayerManagers) {
      ...
      const relayerMangerInstance = new EVMRelayerManager({
        ...
        options: {
          ...
          fundingBalanceThreshold: ethers.utils.parseEther(
      ...
      log.info(
        `Relayer address list length: ${
          addressList.length
        } and minRelayerCount: ${JSON.stringify(
    ...
    for (const type of supportedTransactionType[chainId]) {
      if (type === TransactionType.AA) {
        ...
        // eslint-disable-next-line max-len
        bundlerGasEstimationServiceMap[chainId] =
          new BundlerGasEstimationService(networkService);
        log.info(
