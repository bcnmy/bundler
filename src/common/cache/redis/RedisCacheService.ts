...
export class RedisCacheService implements ICacheService {
   ...
   */
  async decrement(key: string, decrementBy = 1): Promise<boolean> {
  ...
  async get(key: string): Promise<string> {
    try {
      ...
      log.error(
        `Error getting value for key ${key} - ${JSON.stringify(error)}`,
      );
    }
    return "";
  }
   ...
   * @returns true or false basis on success or failure
   */
  async increment(key: string, incrementBy = 1): Promise<boolean> {
    log.info(`Checking if the key: ${key} exists`);
    try {
      const val = await this.redisClient.get(key);
      if (!val) {
  ...
  async set(
    ...
    try {
      ...
      log.error(
        `Error setting value ${value} for key ${key} - ${JSON.stringify(
          error,
        )}`,
      );
      return false;
