  ...
  async getGasPrice(
    ...
    if (this.EIP1559SupportedNetworks.includes(this.chainId)) {
      ...
      if (
        ...
        Number(result.maxPriorityFeePerGas) < 30000000000
      ) {
    ...
    if (
      ...
      try {
        ...
        return {
          ...
          maxPriorityFeePerGas:
            ethers.BigNumber.from("0x3b9aca00").toHexString(),
        };
      }
    }
      ...
      if (!gP) {
        ...
        if (this.EIP1559SupportedNetworks.includes(this.chainId)) {
          if (OptimismNetworks.includes(this.chainId)) {
            try {
              const { data } = await this.networkService.sendRpcCall(
                "eth_gasPrice",
                [],
              );
              const maxFeePerGas = ethers.utils.formatUnits(data.result, "wei");
              await this.setMaxPriorityFeeGasPrice(
                GasPriceType.DEFAULT,
              ...
              log.error(
                `Error in getting network gas price from RPC: ${parseError(
                  error,
                )}`,
              );
              await this.setMaxPriorityFeeGasPrice(
              ...
              await this.setMaxFeeGasPrice(GasPriceType.DEFAULT, "250000000");
            }
            ...
            const maxFeePerGasFromNetwork = (
              await this.networkService.getEIP1559GasPrice()
            ).maxFeePerGas;
            const maxPriorityFeePerGasFromNetwork = (
              await this.networkService.getEIP1559GasPrice()
            ).maxPriorityFeePerGas;
