    ...
    IERC20NetworkService
{
        ...
        switch (tag) {
          ...
          case RpcMethod.getTransactionCount:
            if (params.pendingNonce === true) {
              return await this.ethersProvider.getTransactionCount(
                params.address,
                "pending",
              );
            }
          ...
          case RpcMethod.getTransaction:
            return await this.ethersProvider.getTransaction(
              params.transactionHash,
            );
          default:
            return null;
  ...
  async getEIP1559GasPrice(): Promise<Type2TransactionGasPriceType> {
    ...
    // }
    const maxPriorityFeePerGas = L2Networks.includes(this.chainId)
      ? "0x00"
      : ethers.utils.hexValue(feeData.maxPriorityFeePerGas);
    return {
      maxFeePerGas,
  ...
  async getGasPrice(): Promise<Type0TransactionGasPriceType> {
    const gasPrice = (
      await this.useProvider(RpcMethod.getGasPrice)
    ).toHexString();
    return {
      gasPrice,
    };
  ...
  async getTokenBalance(
    ...
    const erc20Contract = this.getContract(
  ...
  async checkAllowance(
    ...
    const tokensLeftToSpend = await erc20Contract.allowance(
      ownerAddress,
      spenderAddress,
    );
  ...
  async executeReadMethod(
    ...
    const contractReadMethodValue = await contract[methodName].apply(
      null,
      params,
    );
