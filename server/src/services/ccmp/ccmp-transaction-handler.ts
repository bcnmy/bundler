...
import { generateTransactionId } from '../../utils/cache-utils';
...
class CCMPTransactionHandler {
  ...
  constructor(rpcUrl: string) {
    console.log('attempting to create a network sdk instance using rpc', rpcUrl);
  ...
  async handleMessage(txHash: string, gasUsage: number, ccmpEventData: CCMPMessage) {
    ...
    let msgVerificationData;

    await CCMPTransactionHandler.delay(5000);
        ...
        while (resp.status !== GMPStatus.DEST_EXECUTED) {
          ...
          if (resp.gasPaidInfo?.status !== GasPaidStatus.GAS_PAID) {
            ...
            const options: AddGasOptions = {
              ...
              // the sdk will calculate the amount to be paid.
              ...
              // estimatedGasUsed: gasUsage, // An amount of gas to execute `executeWithToken` or
              // `execute` function of the custom destination contract.
              // If not specified, the default value is 700000.

              evmWalletDetails: {
                useWindowEthereum: false,
                privateKey: '6fa6d23734d9aa0b86bc69d1956bc644050e7d1237096ea06bdd3f9b2ce3bcfa',
              },
              // "0xe258a76e1e0abac08e57e67542087f04dd119a73a5f5dec879195fa9b313970e" },
              // A wallet to send an `addNativeGas` transaction. If not specified,
              // the default value is { useWindowEthereum: true}.
            };
          ...
          await CCMPTransactionHandler.delay(10000);
        }
    ...
    //   type: 2, // TODO: Check against destination chain
    ...
    const toChainId = Number(ccmpEventData.destinationChainId);

    // const transactionData: CCMPTransactionMessageType = {
    //   transactionId: txHash,
    //   type: '2', // TODO: Check against destination chain
    //   to: ccmpEventData.destinationGateway,
    //   gasLimit: '0xF4240', // String(simulatedGasLimit),
    ...
    const response = await routeTransactionToRelayerMap[toChainId][
      ...
      chainId: toChainId,
      value: '0x0',
      transactionId: generateTransactionId(txHash),
    });
    if (isError(response)) {
  ...
  //   const CCMPMessageRoutedTopic =
  //     '0xb163bf360fef1e41e4f6cecd0a3e58913c8abcbc45c112f1fb963f2ac9d047e5';
