...
const getUserOperationStateData = async (chainId: number, userOpHash: string): Promise<{
  error: boolean,
  result: any
}> => {
  ...
  if (userOperationStateData.state === UserOperationStateEnum.DROPPED_FROM_BUNDLER_MEMPOOL) {
    log.info(`userOp dropped from mempool with userOpHash: ${userOpHash} on chainId: ${chainId}`);
    return {
      error: false,
      result: {
  ...
  if (userOperationStateData.state === UserOperationStateEnum.BUNDLER_MEMPOOL) {
    log.info(`userOp with userOpHash: ${userOpHash} is in ${UserOperationStateEnum.BUNDLER_MEMPOOL} state on chainId: ${chainId}`);
    return {
      error: false,
      result: {
  ...
  if (userOperationStateData.state === UserOperationStateEnum.SUBMITTED) {
    log.info(`userOp with userOpHash: ${userOpHash} is in ${UserOperationStateEnum.SUBMITTED} state on chainId: ${chainId}`);
    return {
      error: false,
      result: {
  ...
  if (userOperationStateData.state === UserOperationStateEnum.FAILED) {
    log.info(`userOp with userOpHash: ${userOpHash} is in ${UserOperationStateEnum.FAILED} state on chainId: ${chainId}`);
    return {
      error: false,
      result: {
  ...
  if (userOperationStateData.state === UserOperationStateEnum.CONFIRMED) {
    log.info(`userOp with userOpHash: ${userOpHash} is in ${UserOperationStateEnum.CONFIRMED} state on chainId: ${chainId}`);
    log.info(`Fetching userOpReceipt for userOpHash: ${userOpHash} on chainId: ${chainId}`);
  ...
  log.info(`No state matched for userOpHash: ${userOpHash} on chainId: ${chainId}`);
  return {
    error: true,
    result: {
...
export const getUserOperationStatus = async (req: Request, res: Response) => {
  try {
    ...
    const userOperationStateData = await getUserOperationStateData(parseInt(chainId, 10), userOpHash);
