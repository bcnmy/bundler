...
const getStartTimeAndEndTimeInMs = (startTime: string | null, endTime: string | null) => {
 ...
 */
export const getUserOperationsByApiKey = async (req: Request, res: Response) => {
  try {
    ...
    const {
      startTimeInMs,
      endTimeInMs,
    } = getStartTimeAndEndTimeInMs(startTime, endTime);
    log.info(`startTimeInMs: ${startTimeInMs}, endTimeInMs: ${endTimeInMs} for bundlerApiKey: ${bundlerApiKey} on chainId: ${chainId}`);
    ...
    log.info(`getUserOperationsByApiKey request received for bundlerApiKey: ${bundlerApiKey} on chainId: ${chainId}`);

    const userOperationsData = await userOperationDao.getUserOperationsDataByApiKey(
      parseInt(chainId, 10),
      bundlerApiKey,
      startTimeInMs,
      endTimeInMs,
    ...
    if (!userOperationsData) {
      log.info(`User operations data could not for apiKey: ${bundlerApiKey} on chainId: ${chainId}`);
